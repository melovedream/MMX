  ...
  <div class="wrapper">
    ...
    <div class="content">
      ...
      <div class="panel new">
        ...
        <?php
          $coon=mysqli_connect('127.0.0.1','root','root','db_baixiu');
          if(!$coon){
            die('连接失败');}
          $sql='SELECT categories.name,posts.title,posts.created,users.nickname,posts.content,posts.views,
            posts.likes,posts.feature,(SELECT count(id) FROM comments WHERE comments.post_id=posts.id) AS commentscount
              from posts
            ...
            LIMIT 5';
            $sretu=mysqli_query($conn,$sql);
            $starr=[];
            while ( $rows=mysqli_fetch_assoc($sretu)) {
              $starr[]=$rows;
            }
            foreach ($starr as $v) {?>
             <div class="entry">
